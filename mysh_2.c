...
int main(int ac, char * av[]){
	...
	while(1){
    	...
    	*charbuf = 10;
		...
		int sizetoken = parse(charbuf,inputToken);
		...
		//write(1,inputToken[sizetoken-1],sizeof(inputToken[sizetoken-1]));
		fork_rv = 0;
		fork_rv = fork();
		if (fork_rv == 0){
		    if(strcmp(inputToken[sizetoken-1],"&") == 0){
		        inputToken[sizetoken-1] = NULL;
		        fork_rv2 = fork();
		        if(fork_rv2 == 0){
